...
Use this guide to learn how to add agentic AI features to a Chi backend. You will build **Bargain Chef**: a Genkit HTTP service that streams a recipe field-by-field as the model generates it, calling a tool to look up today's grocery sale prices and grounding the ingredients in the deals.

Chi uses standard `net/http` types throughout, so the HTTP handler returned by `genkit.Handler` plugs in directly with no adapter needed. This guide is backend-only. To consume the streamed output from a UI, pair it with one of the frontend integration guides.
