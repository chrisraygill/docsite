...
Use this guide to learn how to add agentic AI features to a Django app. You will build **Bargain Chef**: a Django backend that streams a recipe field-by-field as the model generates it, calling a tool to look up today's grocery sale prices and grounding the ingredients in the deals.

This guide uses [Django Ninja](https://django-ninja.dev) for the HTTP layer. It's async-first and Pydantic-native, which aligns naturally with how Genkit flows are typed. The flow exposes both a JSON endpoint and a Server-Sent Events stream so any client can consume the partial recipe as it arrives.
...
```bash
uv add django django-ninja uvicorn genkit genkit-plugin-google-genai
```
