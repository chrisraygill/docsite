---
title: Hono
description: Start from a Hono server and expose the same recipe flow from the main quickstart using the fetch plugin.
supportedLanguages:
  - js
---
...
<PackageManagerTabs command="npm install genkit @genkit-ai/google-genai @genkit-ai/fetch hono @hono/node-server" />
...
const app = new Hono();
const handleFlow = fetchHandlers([recipeGeneratorFlow], '/api');
