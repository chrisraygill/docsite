...
Use this guide to learn how to build a standalone Genkit backend with Express. You will build **Bargain Chef**: an Express server that streams a recipe field-by-field as the model generates it, calling a tool to look up today's grocery sale prices and grounding the ingredients in the deals.
...
<PackageManagerTabs
  command={`
npm install express genkit @genkit-ai/google-genai @genkit-ai/express @genkit-ai/middleware
npm install -D typescript tsx @types/node @types/express genkit-cli
`}
/>
...
`expressHandler` adapts your Genkit flow to an Express request handler. It parses the JSON request body, invokes the flow, and (when the client opts in with an `Accept: text/event-stream` header) streams chunks back as server-sent events.
