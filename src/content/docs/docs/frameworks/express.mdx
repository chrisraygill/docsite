---
title: Express
description: Start from an Express server and expose the same recipe flow from the main quickstart as an HTTP API.
supportedLanguages:
  - js
---
...
<PackageManagerTabs command="npm install express genkit @genkit-ai/google-genai @genkit-ai/express" />
...
<PackageManagerTabs command="genkit start -- npx tsx --watch src/index.ts" />
...
### Inspect traces from a connected frontend

While the backend is running under `genkit start`, every flow execution is captured as a trace, including requests triggered by a separate frontend (such as an Angular, Next.js, or Flutter app) hitting your endpoint. Open the **Traces** tab in the Developer UI to inspect inputs, outputs, and each step of the generation, and to re-run any past execution with modified inputs.
