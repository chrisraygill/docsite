---
title: FastAPI
description: Start from a FastAPI app and expose the same recipe flow from the main quickstart as an HTTP endpoint.
supportedLanguages:
  - python
---
...
Use this guide when you're starting from a FastAPI app and want the fastest path to serving a Genkit flow from a Python API.
...
mkdir my-genkit-fastapi
cd my-genkit-fastapi
...
Create `main.py` with the same recipe flow used in the main quickstart, then expose it through FastAPI:
...
@app.post('/recipeGeneratorFlow', response_model=None)
