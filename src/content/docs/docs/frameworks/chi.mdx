---
title: Chi
description: Start from a Chi router and expose the same recipe flow from the main quickstart as an HTTP API.
supportedLanguages:
  - go
---
...
Use this guide when you're starting from a Chi router and want the quickest path to a working Genkit integration. Chi uses standard `net/http` types throughout, so Genkit flow handlers plug in directly with no adapter needed.
...
Create a `main.go` file with the same recipe flow used in the main quickstart, then expose it through Chi:
...
func main() {
	...
	r.Post("/recipeGeneratorFlow", genkit.Handler(recipeGeneratorFlow))
